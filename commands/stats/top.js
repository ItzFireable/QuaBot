            ...
            if (!error && body.status == 200) {
                ...
                request.get(latest, { json: true }, (error1, response1, body1) => {
                    ...
                    if (!error1 && body1.status == 200 && body1.scores[0] != undefined) {
                        ...
                        embed.setDescription(`**[(${emojiFlags.countryCode(body.user.info.country).emoji} ${keymodeString}) ${name}'s top scores](https://quavergame.com/profile/${body.user.info.id}?mode=${keysmode})**`);
                        //embed.setAuthor(`${emojiFlags.countryCode(body.user.info.country).emoji} Country Rank: #` + keymodeObject.countryRank);
                        embed.setTimestamp()
